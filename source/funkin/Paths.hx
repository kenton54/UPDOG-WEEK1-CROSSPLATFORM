...
class Paths
{
	...
	static public function textureAtlas(key:String, ?library:String)
	{
		...
		if (OpenFlAssets.exists(modp)) return modp;
	...
	{
		var modFont:String = modsFont(key);
		if (OpenFlAssets.exists(modFont))
			return modFont;
	...
	inline static public function getSparrowAtlas(key:String, ?library:String):FlxAtlasFrames
	{
		...
		if (!OpenFlAssets.exists(xmlFile))
		{
			xmlFile = xml(key, library);
		}

		return FlxAtlasFrames.fromSparrow(image(key, library), xmlFile);
		#end
	}
	...
	inline static public function getPackerAtlas(key:String, ?library:String)
	{
		...
		#else
		var txtFile:String = modsTxt(key);
		if (!OpenFlAssets.exists(txtFile))
		{
			txtFile = file('$key.txt', library);
		}

		return FlxAtlasFrames.fromSpriteSheetPacker(image(key, library), txtFile);
		#end
	}
	...
	public static function returnGraphic(key:String, ?library:String, ?allowGPU:Bool = true)
	{
		...
		// first check mods folder
		var file:String = modsImages(key);
		...
		// if bitmap is still null, check base folder
		if (bitmap == null)
		{
		...
		if (bitmap != null)
		{
			var retVal:Null<FlxGraphic> = cacheBitmap(file, bitmap, allowGPU);
	...
	static public function cacheBitmap(file:String, ?bitmap:BitmapData = null, ?allowGPU:Bool = true):Null<FlxGraphic>
	{
		if (bitmap == null)
		{
			#if MODS_ALLOWED
	...
	public static function readDirectory(path:String):Array<String>
	{
		...
		var files:Array<String> = OpenFlAssets.list().filter(file -> file.startsWith(path));
		return files.map(file -> file.replace(path, ''));
		#end
	}

	inline public static function strip(path:String)
	{
		return path.indexOf(':') != -1 ? path.substr(path.indexOf(':') + 1, path.length) : path;
	}

	inline static public function getModsPath(key:String = ''):String
	{
